...
export default function Login() {
  ...
  // Get the constant OTP: last 6 digits of the emergency number
  const getConstantOtp = () => {
    const digitsOnly = number.replace(/\D/g, '');
    return digitsOnly.slice(-6).padStart(6, '0');
  };

  // Show OTP step (no Firebase — OTP is the last 6 digits of emergency number)
  const triggerSendOtp = async () => {
    setIsSendingOtp(true);
    setOtpError('');

    try {
      // No Firebase call needed — OTP is constant (last 6 digits of emergency number)
      setOtpStep(true);
      setResendTimer(30);
      setOtp(['', '', '', '', '', '']);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!otpStep) {
      // Step 1: Validate details, then go to OTP step
      ...
      if (role === 'caretaker') {
        ...
        // Patient → show OTP screen
        await triggerSendOtp();
      }
    } else {
      // Step 2: Verify OTP against last 6 digits of emergency number
      ...
      const correctOtp = getConstantOtp();

      setIsVerifying(true);
      try {
        if (enteredOtp === correctOtp) {
          // OTP matches — proceed to app
          setLanguage(language);
          setRole(role);
          setUsername(username.trim());
          setAge(age);
          ...
          setOtpError(t(language, 'otpInvalid') || 'Incorrect OTP. Please try again.');
        }
      } finally {
